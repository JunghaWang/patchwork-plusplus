...
class GroundSegmentationServer : public rclcpp::Node {
 public:
  /// GroundSegmentationServer constructor (deleted default constructor)
  GroundSegmentationServer() = delete;
  explicit GroundSegmentationServer(const rclcpp::NodeOptions &options);

 private:
  /// Register new frame
  // 1) 콜백 파라미터를 SharedPtr 로 수정
  void EstimateGround(sensor_msgs::msg::PointCloud2::SharedPtr msg);
